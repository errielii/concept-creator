...
titleGradient.onclick=()=>{
  ...
  const active=
    title.classList.toggle('title-custom-gradient');
...
document.getElementById('add').onclick=()=>{
  const name=
    document.getElementById('statName').value.trim() ||
    'NEW STAT';
...
function exportFileName(ext){
  const style=(
    document.getElementById('styleName').value.trim() ||
    document.getElementById('styleTitle').textContent.trim() ||
    'Name'
  )
  .replace(/[\\/:*?"<>|]/g,'')
  .trim() || 'Name';
...
document.getElementById('savePng').onclick=async()=>{
  ...
  try{
    ...
    stats.querySelectorAll('.remove')
      .forEach(x=>x.remove());

    stats.querySelectorAll('.fill')
      .forEach(fill=>{
        fill.style.left='3px';
        fill.style.top='3px';
    ...
    const isGradient=
      title.classList.contains('title-gradient') ||
